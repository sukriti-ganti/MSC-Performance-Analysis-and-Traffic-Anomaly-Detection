...
Generates:
  ...
  - Per-class precision/recall/F1 bar chart per model
...
CLASS_NAMES = ["Normal", "Abnormal", "Drone"]
...
for model_name, pkl_path in MODELS.items():
    ...
    metrics_df.T.plot(kind="bar", figsize=(7, 4), ylim=(0, 1.12),
...
colors = ["#e74c3c" if f in ["doppler_shift_hz","velocity_mps","fading_rate",
                              "channel_variation","handovers"]
          else "#2E5496" if f in ["packet_loss","avg_delay","interference_dbm",
                                   "active_pcs","traffic_intensity"]
          else "#27AE60"
          for f in fi.head(15).index]

fi.head(15).plot(kind="barh", ax=ax, color=colors, edgecolor="white")
ax.invert_yaxis()
ax.set_title("Random Forest — Feature Importances (Top 15)\n"
             "Blue=NPS/EMTL  Red=Drone signature  Green=Wireless",
             fontsize=12, fontweight="bold")
...
print("\n[evaluate] Summary → results/evaluation_summary.csv")
print("\n[evaluate] All done.")